 ...
 * 
 * OPTIMIZED: Added caching to prevent repeated Firebase calls
 */

import { FDB_DATASET_CONFIG, FDB_APP_NAME, FB_PATHS, getCurrentMonthKey, getCurrentWeekKey } from './config.js';
...
const leaderboardCache = {
  allTime: { data: null, timestamp: 0, ttl: 10 * 60 * 1000 },  // 10 min cache
  monthly: {},  // { 'YYYY-MM': { data, timestamp, ttl } }
  weekly: {}    // { 'YYYY-Wxx': { data, timestamp, ttl } }
};

function isCacheValid(cacheEntry) {
  return cacheEntry && cacheEntry.data && (Date.now() - cacheEntry.timestamp < cacheEntry.ttl);
}

function setCache(cacheEntry, data, ttl = 5 * 60 * 1000) {
...
export async function loadHallOfFame(containerId, highlightUsername = null) {
  ...
  try {
    // Check cache first
    if (isCacheValid(leaderboardCache.allTime)) {
      console.log("📦 Using cached all-time leaderboard");
      renderHallOfFame(container, leaderboardCache.allTime.data, highlightUsername);
      return;
    }
    ...
    // Cache the data
    setCache(leaderboardCache.allTime, leaderboardData, 10 * 60 * 1000);
    
    renderHallOfFame(container, leaderboardData, highlightUsername);
...
async function renderHallOfFame(container, leaderboardData, highlightUsername) {
  ...
  const leaderboard = Array.isArray(leaderboardData) 
    ? leaderboardData.filter(x => x).slice(0, 10)
    : Object.values(leaderboardData).slice(0, 10);
...
export async function loadMonthlyLeaderboard(containerId, highlightUsername = null, monthKey = null) {
  ...
  try {
    ...
    // Check cache
    if (!leaderboardCache.monthly[targetMonth]) {
      leaderboardCache.monthly[targetMonth] = { data: null, timestamp: 0, ttl: 5 * 60 * 1000 };
    }
    
    if (isCacheValid(leaderboardCache.monthly[targetMonth])) {
      console.log(`📦 Using cached monthly leaderboard for ${targetMonth}`);
      renderMonthlyLeaderboard(container, leaderboardCache.monthly[targetMonth].data, highlightUsername);
      return;
    }
    
    console.log(`🔄 Fetching monthly leaderboard for ${targetMonth}...`);
    ...
    const data = snap.val();
    setCache(leaderboardCache.monthly[targetMonth], data);
    renderMonthlyLeaderboard(container, data, highlightUsername);
...
function renderMonthlyLeaderboard(container, data, highlightUsername) {
  ...
  if (userEntry && !top10.find(t => t.username === userEntry.username)) {
    html += `
...
export async function loadWeeklyLeaderboard(containerId, highlightUsername = null, weekKey = null) {
  ...
  try {
    ...
    // Check cache
    if (!leaderboardCache.weekly[targetWeek]) {
      leaderboardCache.weekly[targetWeek] = { data: null, timestamp: 0, ttl: 5 * 60 * 1000 };
    }
    
    if (isCacheValid(leaderboardCache.weekly[targetWeek])) {
      console.log(`📦 Using cached weekly leaderboard for ${targetWeek}`);
      renderWeeklyLeaderboard(container, leaderboardCache.weekly[targetWeek].data, highlightUsername);
      return;
    }
    
    console.log(`🔄 Fetching weekly leaderboard for ${targetWeek}...`);
    ...
    const data = snap.val();
    setCache(leaderboardCache.weekly[targetWeek], data);
    renderWeeklyLeaderboard(container, data, highlightUsername);
...
function renderWeeklyLeaderboard(container, data, highlightUsername) {
  // Handle both array format (new) and object format (legacy)
  let list = Array.isArray(data) 
    ? data.filter(x => x).map(info => ({
        username: info.username,
...
export function invalidateLeaderboardCache(type = 'all') {
  if (type === 'all' || type === 'allTime') {
    leaderboardCache.allTime.timestamp = 0;
  }
  ...
  console.log(`🗑️ Leaderboard cache invalidated: ${type}`);
}
